...
const filterByRecentDays = (history: HistoryItem[]): HistoryItem[] => {
...
export const calculateVolatility = (history: HistoryItem[]): number => {
  const filteredHistory = filterByRecentDays(history);
  if (filteredHistory.length < 2) return 0;
  const scores = filteredHistory.filter((h) => h.time).map((h) => h.score);
...
export const calculateMomentum = (history: HistoryItem[]): number => {
  const filteredHistory = filterByRecentDays(history);
  if (filteredHistory.length < 7) return 0;

  const validHistory = filteredHistory.filter((h) => h.time);
...
export const calculateMediaFrequency = (feed: NewsEvent[], politicianId: string): number => {
  ...
  sixtyDaysAgo.setDate(sixtyDaysAgo.getDate() - DAYS_TO_KEEP);

  return feed.filter((event) => {
    const eventDate = new Date(event.timestamp);
    return event.politicianId === politicianId && eventDate >= sixtyDaysAgo;
  }).length;
};
...
export const calculateInfluenceScore = (
  ...
  const influence =
    (scoreComponent / 100) * 0.5 + (avgWeight / 3) * 0.3 + (Math.min(eventCount, 20) / 20) * 0.2;
...
export const calculateTrendStrength = (history: HistoryItem[]): number => {
  ...
  const scores = filteredHistory.slice(-14).map((h) => h.score);
...
export const calculateConsistencyScore = (history: HistoryItem[]): number => {
  const filteredHistory = filterByRecentDays(history);
  if (filteredHistory.length < 5) return 0;
...
export const calculateSentimentBreakdown = (
  history: HistoryItem[]
): AdvancedMetrics['sentimentBreakdown'] => {
...
export const calculateAnalyticsSummary = (
  ...
  const topMover = politicians.reduce(
    (best, p) => {
...
export const importData = (
  jsonString: string
): { politicians: Politician[]; feed: NewsEvent[]; sources: Source[] } | null => {
  try {
